...
def setup_presidio():
    ...
    if _ANALYZER_CACHE is not None and _ANONYMIZER_CACHE is not None:
    ...
    _ANALYZER_CACHE  = AnalyzerEngine(registry=registry)
    _ANONYMIZER_CACHE = AnonymizerEngine()
    return _ANALYZER_CACHE, _ANONYMIZER_CACHE


def pii_scan(text: str, analyzer=None, anonymizer=None, target_entities: list[str] | None = None) -> dict:
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    """
    # Fast regex/keyword check for explicit adversarial signals
    jailbreak_signals = [
        "bỏ qua tất cả", "ignore your previous", "ignore all", "pretend you are dan",
        "unrestricted ai", "forget your system", "system override",
        "đóng vai ai", "không có giới hạn", "bảng lương chi tiết",
        "tấn công mạng", "admin command", "dump all training data", "tôi là ceo",
        "in ra toàn bộ system instructions", "<!-- ignore", "tiết lộ bảng lương",
        "cccd và số điện thoại", "thông tin cá nhân của"
    ]
    offtopic_signals = [
        "bài thơ", "nấu phở", "bitcoin", "ethereum", "phương trình vi phân", "marvel",
        "công thức truyền thống"
    ]
    lower_t = text.lower()
    if any(sig in lower_t for sig in jailbreak_signals) or any(sig in lower_t for sig in offtopic_signals):
        return {
            "allowed": False,
    ...
    try:
        ...
        refuse_keywords = [
            ...
            "từ chối", "ngoài phạm vi", "không cung cấp"
        ]
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    """
    sensitive_keywords = ["mật khẩu admin", "cccd", "cmnd", "034095", "0987654321", "bảng lương chi tiết"]
...
def save_guard_report(adv_results: list[dict], latency: dict, path: str = "reports/guard_results.json"):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    passed = sum(1 for r in adv_results if r["passed"])
    report = {
        "total_adversarial": len(adv_results),
        "passed_count": passed,
        "pass_rate": round(passed / max(len(adv_results), 1), 4),
